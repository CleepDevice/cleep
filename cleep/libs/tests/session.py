...
class AnyArg(object):
    ...
    Deprecated: please use same class from libs.tests.common
    """
...
class TestSession:
    ...
    def __build_bootstrap_objects(self, debug):
        ...
        critical_resources = CriticalResources(
            {"task_factory": self.task_factory}, debug
        )
        ...
        for item in traceback.extract_stack():
            if os.path.basename(item.filename).startswith("test_"):
                return (
                    item.name
                    if item.name != "<module>"
                    else "<test case name hidden by coverage>"
                )
    ...
    def clean(self):
        ...
        if self.__module_instance and self.__module_instance.is_alive():
            ...
            try:
                ...
                raise Exception(
                    "TEST: Please don't forget to start module application before running unit test (session.start_module())"
                ) from error

        # config
        if hasattr(self.__module_instance, "MODULE_CONFIG_FILE"):
            path = os.path.join(
                self.__module_instance.CONFIG_DIR,
                self.__module_instance.MODULE_CONFIG_FILE,
            )
    ...
    def make_mock_command(
        self, command_name, data=None, fail=False, no_response=False, side_effect=None
    ):
        """
    ...
    def set_mock_command_succeed(self, command_name):
        ...
        if command_name not in self.__bus_command_handlers:
            raise Exception(
                f'"{command_name}" not mocked. Please mock it first (session.add_mock_command)'
            )
    ...
    def command_called_with(self, command_name, params, to=None):
        ...
        """
        if command_name in self.__bus_command_handlers:
            params_check = self.__check_command_called_with_params(
                self.__bus_command_handlers[command_name], params
            )
            params_error = (
                "  Expected params: %s\n  Received params: %s"
                % (params, self.__bus_command_handlers[command_name]["params"])
                if not params_check
                else ""
            )
            to_check = (
                True
                if to is None
                else self.__bus_command_handlers[command_name]["lastto"] == to
            )
            to_error = (
                "  Expected to: %s\n  Current to: %s"
                % (to, self.__bus_command_handlers[command_name]["lastto"])
                if not to_check
                else ""
            )
            ...
            if not check:
                self.logger.fatal(
                    "TEST: command_called_with failed:\n%s%s%s"
                    % (params_error, ("\n" if params_error else ""), to_error)
                )
            return check

        self.logger.fatal(
            'TEST: command_called_with failed: command not mocked in "%s", please mock it using session.add_mock_command'
            % self.test_case_name
        )
        return False
    ...
    def assert_command_not_called(self, command_name):
        ...
        if self.__bus_command_handlers[command_name]["calls"] != 0:
            self.testcase.fail(
                'Command "%s" was called while it should not' % command_name
            )
    ...
    def assert_command_called_with(self, command_name, params, to=None):
        ...
        if len(self.__bus_command_handlers[command_name]["params"]) == 1:
            if type(params) is not type(
                self.__bus_command_handlers[command_name]["params"][0]
            ):
                self.testcase.fail(
                    'Command "%s" parameters differs: %s is not %s (%s)'
                    % (
                        command_name,
            ...
            if not params_check:
                self.testcase.fail(
                    'Command "%s" was not called with awaited parameters\n  Expected params: %s\n  Received params: %s'
                    % (
                        command_name,
                        params,
                        self.__bus_command_handlers[command_name]["params"],
    ...
    def assert_event_not_called(self, event_name):
        ...
        self.testcase.assertTrue(
            self.event_call_count(event_name) == 0,
            'Event "%s" was called while it should' % event_name,
        )
    ...
    def assert_event_called(self, event_name):
        ...
        self.testcase.assertTrue(
            self.event_call_count(event_name) > 0,
            'Event "%s" was not called while it should' % event_name,
        )
    ...
    def assert_event_called_with(self, event_name, params, device_id=None):
        ...
        self.testcase.assertTrue(
            self.event_call_count(event_name) > 0,
            'Event "%s" was not called' % event_name,
        )
        last_params = self.get_last_event_params(event_name)
        self.testcase.assertDictEqual(
            params, last_params, 'Event "%s" parameters are differents' % event_name
        )
    ...
    def _internal_bus_push_mock(self, request, timeout):
        ...
        if request and request.command in self.__bus_command_handlers:
            ...
            self.__bus_command_handlers[request.command]["params"].append(
                request.params
            )
            self.__bus_command_handlers[request.command][
                "lastdeviceid"
            ] = request.device_id
            ...
            if self.__bus_command_handlers[request.command]["fail"]:
                self.logger.debug(
                    'TEST: command "%s" fails for tests' % request.command
                )
                raise CommandError(
                    'TEST: command "%s" fails for tests' % request.command
                )
                ...
                self.logger.debug(
                    'TEST: command "%s" returns no response for tests' % request.command
                )
                raise NoResponse(
                    request.to,
                    request.timeout,
                    'TEST: no response for command "%s"' % request.command,
                )

            cmd_data = self.__bus_command_handlers[request.command]["data"]
            cmd_side_effect = self.__bus_command_handlers[request.command][
                "side_effect"
            ]
            if cmd_data is None and cmd_side_effect is None:
                ...
                if len(cmd_side_effect) > 0:
                    ...
                    if isinstance(data, Exception):
                        self.logger.debug(
                            'TEST: command "%s" fails for tests with custom "%s" exception',
                            request.command,
                        ...
                        self.logger.debug(
                            'TEST: command "%s" fails for test', request.command
                        )
        ...
        self.logger.fatal(
            ...
            % (request.command, self.test_case_name)
        )
        raise Exception(
            'TEST: command "%s" is not handled in "%s", please mock it using session.add_mock_command'
            % (request.command, self.test_case_name)
