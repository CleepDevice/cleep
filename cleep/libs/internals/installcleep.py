...
class InstallCleep(threading.Thread):
    ...
    This class download latest archive from cleep repository, executes prescript, installs deb package
    ...
    def __download_checksum(self, download, url_checksum):
        ...
        try:
            ...
            self.logger.debug(u'Cleep archive checksum: %s' % checksum)
    ...
    def __extract_archive(self, archive_path):
        ...
        try:
            ...
            self.logger.exception(u'Error decompressing cleep archive "%s" in "%s":' % (archive_path, extract_path))
            self.crash_report.report_exception()
            extract_path = None
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                if not self.__execute_script(path):
                    ...
                    self.crash_report.manual_report(u'Cleep update failed: prescript failed', self.__pre_script_status)
                    error = True
                else:
        ...
        try:
            ...
            if os.path.exists(path):
                ...
                if not self.__execute_script(path):
                    ...
                    self.crash_report.manual_report(u'Cleep update failed: postscript failed', self.__post_script_status)
                    error = True
                else:
    ...
    def __install_deb(self, extract_path):
        ...
        try:
            # prepare installer
            deb_path = os.path.join(extract_path, u'cleep.deb')
