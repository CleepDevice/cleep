...
class EventsBroker():
            ...
            for filename in filenames:
                ...
                try:
                    ...
                    if filename.lower().find(u'event')>=0 and ext==u'.py':
                        self.logger.debug('Loading "%s"' % u'%s%s.%s' % (self.PYTHON_CLEEP_IMPORT_PATH, parts[-2], event))
                        mod_ = importlib.import_module(u'%s%s.%s' % (self.PYTHON_CLEEP_IMPORT_PATH, parts[-2], event))
    ...
    def get_module_events(self, module_name):
        ...
        if module_name not in self.events_by_module.keys():
            raise Exception(u'Module name "%s" is not referenced in Cleep' % module_name)
