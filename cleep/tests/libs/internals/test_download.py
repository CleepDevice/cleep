...
class DownloadTests(unittest.TestCase):
    ...
    def init(self, url='https://www.google.com', resp_data='', resp_status=200, resp_headers={}, stream=False):
        responses.add(
            responses.GET,
            url,
            body=resp_data,
    ...
    def test_download_file_async(self):
        ...
        self.init(
            resp_data=response,
            stream=True,
            resp_headers={'Content-Type': 'application/octet-stream', 'Content-Length': '%s' % len(response.encode('utf8'))}
        )
        self.d.download_file_async('https://www.google.com', self._end_callback, status_callback=self._status_callback)
        time.sleep(1)
    ...
    def test_download_file_async_with_cached_filename(self):
        ...
        self.d.download_file_async('https://www.google.com', self._end_callback, status_callback=self._status_callback, cache_filename='mycachedfile')
        time.sleep(0.5)
        self.assertEqual(self.async_status, self.d.STATUS_DONE)
    ...
    def test_download_file_async_with_cached_file(self):
        try:
            self.init()
            create_files_tree(self.d.DOWNLOAD_FILE_PREFIX, self.d.CACHED_FILE_PREFIX)
            self.d.download_file_async('https://www.google.com', self._end_callback, status_callback=self._status_callback, cache_filename='123456789')
            time.sleep(.25)
    ...
    def test_download_file_async_check_sha1(self):
        ...
        self.d.download_file_async('https://www.google.com', self._end_callback, check_sha1='1234', status_callback=self._status_callback)
        time.sleep(0.5)
        self.assertEqual(self.async_status, self.d.STATUS_DONE)
    ...
    def test_download_file_async_check_sha256(self):
        ...
        self.d.generate_md5 = Mock()
        self.d.download_file_async('https://www.google.com', self._end_callback, check_sha256='1234', status_callback=self._status_callback)
        time.sleep(0.5)
        self.assertEqual(self.async_status, self.d.STATUS_DONE)
    ...
    def test_download_file_async_check_md5(self):
        ...
        self.d.download_file_async('https://www.google.com', self._end_callback, check_md5='1234', status_callback=self._status_callback)
        time.sleep(0.5)
        self.assertEqual(self.async_status, self.d.STATUS_DONE)
        self.assertFalse(self.d.generate_sha1.called)
        self.assertFalse(self.d.generate_sha256.called)
    ...
    def test_download_file_async_invalid_size(self):
        ...
        self.init(
            resp_data=response,
            stream=True,
            resp_headers={'Content-Type': 'application/octet-stream', 'Content-Length': '%s' % (len(response.encode('utf8'))-1)}
        )
        self.d.download_file_async('https://www.google.com', self._end_callback, status_callback=self._status_callback)
        time.sleep(1)
        self.assertGreaterEqual(self.status_callback_call, 2)
...
class DownloadTestsNoCleepFilesystem(unittest.TestCase):
    ...
    def _init_download(self, remove_side_effect=None, rename_side_effect=None, open_side_effect=None,
                       url='https://www.google.com', resp_data='', resp_status=200, resp_headers={}, stream=False):
        responses.add(
            responses.GET,
            url,
            body=resp_data,
