...
class EventsBrokerTests(unittest.TestCase):
    ...
    def _init_context(self, event_content=EVENT_CONTENT, add_apps=True, add_core=False):
        ...
        if add_core:
            ...
            with io.open(os.path.join(app_dir, "%s.py" % event_class), "w") as fd:
                fd.write(
                    event_content
                    % {"event_class": event_class, "event_name": "core.event.test"}
                )
    ...
    def test_get_external_bus_name(self):
        ...
        self.assertEqual(
            self.e._EventsBroker__get_external_bus_name(), "externalbusname"
        )
    ...
    def test_load_events_from_core_crash_report(self):
        ...
        self.crash_report.report_exception.assert_called_with(
            {"message": "Invalid core events path", "path": "/tmp/dummy"}
        )

    def test_load_events_from_core_invalid_classname(self):
        self._init_context(
            add_apps=False, add_core=True, event_content=EVENT_CONTENT_INVALID_CLASSNAME
        )
        self.e.logger = Mock()
        ...
        self.e.logger.error.assert_any_call(
            'Event class must have the same name than the filename "%s"',
            "/tmp/test_core/coreEventTest.py"
        )

    def test_load_events_from_core_invalid_syntax(self):
        self._init_context(
            add_apps=False, add_core=True, event_content=EVENT_CONTENT_SYNTAX_ERROR
        )
        self.e.logger = Mock()
        ...
        self.assertEqual(len(self.e.events_by_event), 0)
        self.e.logger.exception.assert_called_with(
    ...
    def test_load_events_from_apps_invalid_classname(self):
        ...
        self.e.logger.error.assert_any_call(
            'Event class must have the same name than the filename "%s"',
            "/tmp/test_modules/app2/app2EventTest.py"
        )
        self.e.logger.error.assert_any_call(
            'Event class must have the same name than the filename "%s"',
            "/tmp/test_modules/app1/app1EventTest.py"
        )
    ...
    def test_load_events_from_apps_invalid_syntax(self):
        ...
        self.assertEqual(len(self.e.events_by_event), 0)
        self.e.logger.exception.assert_any_call(
            'Event "%s" wasn\'t imported successfully. Please check event source code.',
            "app2EventTest",
        )
        self.e.logger.exception.assert_any_call(
    ...
    def test_get_used_events(self):
        ...
        self.assertTrue(
            self.__class__.__name__.lower() in events["test.event.app1"]["modules"]
        )
    ...
    def test_get_module_events_invalid_module(self):
        ...
        self.assertEqual(
            str(cm.exception), 'Module name "dummy" is not referenced in Cleep'
        )
    ...
    def test_set_event_renderable(self):
        ...
        logging.debug("Events not renderable: %s" % self.e.events_not_renderable)
        self.assertTrue(
            "otherrenderer" in self.e.events_not_renderable["test.event.app1"]
        )
